...
- `~/.claude/settings.json` is a live symlink into this repo (Kun Chen's pattern). Claude Code and Orca rewrite that file at runtime (Orca injects its `~/.orca/agent-hooks` hooks), so the working tree gets dirty when they do; commit the keepers, `git checkout` the rest. Never replace the symlink with a copy.
- `rebuild.sh` and `bootstrap.sh` refuse root, linked worktrees and a `~/.dotfiles` link that points elsewhere. A feature branch in the primary clone is allowed with a note. `doctor.sh` explains a failing rebuild; run it before asking the human for the error text.
