...
- `~/.claude/settings.json` is a live symlink into this repo. Claude Code and Orca rewrite that file at runtime (Orca injects its `~/.orca/agent-hooks` hooks), so the working tree gets dirty when they do; commit the keepers, `git checkout` the rest. Never replace the symlink with a copy.
