...
- Orca launches every harness with permission-bypass. `home/.grok/requirements.toml` sets `disable_bypass_permissions_mode = false` and nix-darwin installs it to `/etc/grok/requirements.toml`. Do not flip it to true.
