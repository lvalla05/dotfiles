...
- The Orca cask is `stablyai/orca/orca`, fully qualified. The bare token `orca` is a disabled Plotly cask. Orca is the ADE. Harnesses rotate. Do not treat any CLI as the permanent primary.
...
- Do not write agent-attribution comments in this repo (which model, which session, who generated a line). Session notes, prompts, and life context live in the Obsidian vault, never here.
